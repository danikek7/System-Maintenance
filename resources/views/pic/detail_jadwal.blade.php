...
<head>
    <meta charset="UTF-8">
    <title>Detail Jadwal - PIC Maintenance</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    $bulanFinal = '-';
    try {
        $bulanFinal = \Carbon\Carbon::createFromFormat('Y-m', $jadwal->bulan)->translatedFormat('F');
    } catch (\Exception $e) {
        $bulanFinal = '-';
    }
@endphp
                                    ...
                                    <table class="w-full border-collapse">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-blue-50 transition">
                                                <td class="px-4 py-4 text-sm text-gray-900 border text-center">{{ $index + 1 }}</td>
                                                ...
                                                <td class="px-4 py-4 text-sm text-gray-900 border text-center">
                                                    @switch($detail->status_laporan_code)
                                                        @case(-1)
                                                            <span class="text-gray-500 italic">Belum dibuat</span>
                                                        @break

                                                        @case(0)
                                                            <span class="text-yellow-600 italic">Belum diajukan</span>
                                                        @break

                                                        @case(1)
                                                            <span class="text-blue-600 font-semibold">Menunggu approval</span>
                                                        @break

                                                        @case(2)
                                                        @case(3)
                                                            <span class="text-green-600 font-semibold">Sudah diapprove</span>
                                                        @break

                                                        @default
                                                            <span class="text-gray-400">Status tidak diketahui</span>
                                                    @endswitch
                                                </td>
                                                <td class="px-4 py-4 text-sm text-gray-900 border text-center">
                                                    @if($detail->status_laporan_code == 1)
                                                        <div class="flex justify-center gap-2">
                                                            <form
                                                                action="{{ route('pic.laporan.approveDetail', $detail->id) }}"
                                                                method="POST"
                                                                onsubmit="return confirm('Setujui detail jadwal ini?')">
                                                                @csrf
                                                                <button type="submit"
