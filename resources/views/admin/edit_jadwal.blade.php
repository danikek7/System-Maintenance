...
<head>
    ...
    <title>Edit Jadwal - Maintenance System</title>
...
<body class="bg-gray-100 min-h-screen">
            ...
            <div class="flex-1 p-8 overflow-auto">
                ...
                <div class="max-w-4xl mx-auto bg-white shadow rounded-lg p-6">
                    <form action="{{ route('admin.jadwal.update', $jadwal->id) }}" method="POST">
                        ...
                        <h2 class="text-center text-xl font-bold text-gray-800">EDIT JADWAL</h2><br>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="relative">
                                <input type="month" name="bulan" required
                                    ...
                                    value="{{ old('bulan', \Carbon\Carbon::parse($jadwal->bulan)->format('Y-m')) }}">
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div id="selected-assets-container">
                                @if($jadwal->details->isEmpty())
                                    ...
                                    <div id="assets-table-container" class="hidden"></div>
                                @else
                                    <div id="no-assets-message" class="text-gray-500 text-sm italic hidden">
                                        Belum ada aset yang dipilih
                                    </div>
                                            ...
                                            <table>
                                                ...
                                                <tbody id="selected-assets-body">
                                                    ...
                                                    <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-blue-50 transition">
                                                        ...
                                                        <td class="font-medium">{{ $detail->nama_asset }}</td>
                                                        <td class="font-mono text-blue-600">{{ $detail->asset->asset_tag ?? '-' }}</td>
                                                        <td>{{ $detail->nama_location }}</td>
                                                        <td class="notes-cell" title="{{ $detail->asset->notes ?? '' }}">
                                                            {{ $detail->asset->notes ? Str::limit($detail->asset->notes, 30) : '-' }}
                                                        </td>
                        ...
                        <div id="form-data-container">
                            @if($jadwal->details->isNotEmpty())
                                <input type="hidden" name="lokasi" value="{{ $jadwal->details[0]->location_id }}">
                                @foreach($jadwal->details as $detail)
                                    <input type="hidden" name="aset[]" value="{{ $detail->asset_id }}">
                                @endforeach
                            @endif
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            ...
                            <a href="{{ route('admin.jadwal.index') }}" 
                               class="inline-block px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-100">
                                Batal
                            </a>
                        </div>
                    </form>
    ...
    <script>
        ...
        let selectedAssets = [
            ...
            {
                id: {{ $detail->asset_id }},
                name: "{{ $detail->nama_asset }}",
                asset_tag: "{{ $detail->asset->asset_tag ?? '' }}",
                locationId: {{ $detail->location_id }},
                locationName: "{{ $detail->nama_location }}",
                notes: "{{ $detail->asset->notes ?? '' }}"
            },
            @endforeach
        ];
            ...
            selectedAssets.forEach((asset, index) => {
                html += `
                    <tr class="${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition">
                        ...
                        <td class="notes-cell" title="${asset.notes || ''}">
                            ${asset.notes ? asset.notes.substring(0, 30) + (asset.notes.length > 30 ? '...' : '') : '-'}
                        </td>
                        <td class="text-center">
                            <button 
                                type="button" 
        ...
        function updateFormData() {
            ...
            // Add single location (use the first asset's location)
            const locationInput = document.createElement('input');
            locationInput.type = 'hidden';
            locationInput.name = 'lokasi[]';  // Singular name to match validation
            locationInput.value = selectedAssets[0].locationId;
            formDataContainer.appendChild(locationInput);
            
            // Add asset IDs
            selectedAssets.forEach(asset => {
                const assetInput = document.createElement('input');
                assetInput.type = 'hidden';
                assetInput.name = 'aset[]';  // Match controller validation ('aset')
                assetInput.value = asset.id;
                formDataContainer.appendChild(assetInput);
            });
        }
    </script>
</body>
